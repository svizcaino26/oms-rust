...
use oms_rust::dto::products_dto::NewProduct;
use sqlx::PgPool;
use std::env;
...
async fn main() -> anyhow::Result<()> {
    ...
    match update_product(&db, id, new_price_cents).await? {
        true => println!("Price updated"),
        false => println!("Failed to update price")
    }
